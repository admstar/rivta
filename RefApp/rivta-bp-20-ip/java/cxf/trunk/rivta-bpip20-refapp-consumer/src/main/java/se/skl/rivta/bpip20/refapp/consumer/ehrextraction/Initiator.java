...
package se.skl.rivta.bpip20.refapp.consumer.ehrextraction;
...
import se.skl.riv.ehr.ehrexchange.getehrextract.v1.rivtabpip20.GetEhrExtractResponderInterface;
import se.skl.riv.ehr.ehrexchange.getehrextract.v1.rivtabpip20.GetEhrExtractResponderService;
import se.skl.riv.ehr.ehrexchange.getehrextractresponder.v1.GetEhrExtractRequestType;
import se.skl.riv.ehr.ehrexchange.getehrextractresponder.v1.GetEhrExtractResponseType;
import se.skl.riv.ehr.ehrexchange.getehrextractresponder.v1.PingRequestType;
import se.skl.riv.ehr.ehrexchange.getehrextractresponder.v1.PingResponseType;
import se.skl.riv13606.v2.EHREXTRACT;
import se.skl.riv13606.v2.II;
...
public class Initiator {
	...
	static private final String LOGICAL_ADDRESS = "SE165565968202-3P3Q"; // HSA-id for Carelink AB, assuming the responder service i NPO frage-tjanst	
	static private final AssertionType HP_ASSERTION = new AssertionType();
	
	private GetEhrExtractResponderInterface service = null;
	...
	public static void main(String[] args) {
		...
		long ts = 0;
		logger.info("RIV TA Basic Profile med intygspropagering v2.0 - Ref App, Apache CXF Consumer running on Java version {}", System.getProperty("java.version"));
		...
		ts = System.currentTimeMillis();
		GetEhrExtractResponseType reply = initiator.callGetEhrExtract("RIV TA BP IP 2.0 Ref App OK", LOGICAL_ADDRESS);
		...
		ts = System.currentTimeMillis();
		reply = initiator.callGetEhrExtract("RIV TA BP IP 2.0 Ref App OK", LOGICAL_ADDRESS);
		logger.info("...Producer returned (in {} ms): ", (System.currentTimeMillis() - ts), initiator.getIdentifierName(reply));
	}
	...
	public Initiator(String address) {
		...
		service = new GetEhrExtractResponderService(createEndpointUrlFromServiceAddress(address)).getGetEhrExtractResponderPort();
	}
	...
	public GetEhrExtractResponseType callGetEhrExtract(String id, String logicalAddresss) {
		...
		GetEhrExtractResponseType result = service.getEhrExtract(logicalAddressHeader, HP_ASSERTION, request);
