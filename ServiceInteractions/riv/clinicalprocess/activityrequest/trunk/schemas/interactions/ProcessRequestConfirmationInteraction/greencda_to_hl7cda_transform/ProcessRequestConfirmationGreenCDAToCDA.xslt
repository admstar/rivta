...
-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:ns0="urn:riv:clinicalprocess:activityrequest:1" xmlns:ns1="urn:riv:clinicalprocess:activityrequest:ProcessRequestConfirmationResponder:1" xmlns:ns2="urn:riv:clinicalprocess:activityrequest:request" exclude-result-prefixes="xs fn ns0 ns1 ns2">
	...
	<xsl:template match="/">
		<ClinicalDocument xmlns="urn:hl7-org:v3">
			...
			<xsl:for-each select="ns1:ProcessRequestConfirmation/ns0:requestConfirmation">
				<xsl:variable name="var1_requestOutcomeTime" as="node()?" select="ns2:requestOutcomeTime"/>
				<xsl:variable name="var2_resultof_first" as="node()" select="ns2:patient"/>
				<xsl:variable name="var3_resultof_first" as="node()" select="ns2:requestOutcomeId"/>
				...
				<xsl:variable name="var6_resultof_first" as="node()" select="$var2_resultof_first/ns2:personId"/>
				<xsl:variable name="var7_resultof_first" as="node()" select="$var4_resultof_first/ns2:healthcareProfessional"/>
				<xsl:variable name="var8_resultof_first" as="node()" select="ns2:recipient/ns2:organisation/ns2:careUnitId"/>
				<xsl:variable name="var9_resultof_first" as="node()" select="$var4_resultof_first/ns2:healthcareProfessionalOrganisation/ns2:careUnitId"/>
				<typeId>
				...
				<recordTarget>
					<patientRole>
						...
						<patient>
							<xsl:for-each select="$var2_resultof_first/ns2:name">
								<name>
				...
				<author>
					...
					<assignedAuthor>
						<xsl:for-each select="$var7_resultof_first/ns2:id">
							<id>
						...
						<assignedPerson>
							<name>
								<xsl:sequence select="fn:string($var7_resultof_first/ns2:name)"/>
							</name>
						</assignedPerson>
					</assignedAuthor>
				</author>
					...
					<structuredBody>
						<component>
							<section>
								...
								<title>Övrig svarsinformation</title>
								<text>
									<xsl:sequence select="fn:string(ns2:outcome/ns2:outcomeText)"/>
								</text>
								<component>
									<section>
										<code>
